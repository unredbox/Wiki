...
## Carousel
The carousel consists of a central solid steel rod with 8 plastic platters (decks), each deck consists of 6 "sections" (the area between rods) where each section holds 15 disks, for a total of 90 disks per deck. A VMZ (Vertical Merchandizing Zone) equipped kiosk can store a total of 720 disks (but usually only contains around 600 to keep the VMZ empty and to reserve space for returns).
